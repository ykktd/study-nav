...
<aside class="sticky top-0 flex h-screen flex-col gap-7 border-r border-hairline-soft bg-gradient-to-b from-bg to-bg-deep px-4.5 py-5.5">
	...
	<nav class="flex flex-col gap-0.5" aria-label="メインナビゲーション">
		<div class="my-1 px-[10px] text-[10.5px] uppercase tracking-[0.14em] text-ink-4">Workspace</div>
		<a href="/" class="flex items-center gap-2.5 rounded-lg px-[10px] py-2 text-[13.5px] text-ink-2 no-underline" class:active={isActive('/')}>
			<span class="size-1.5 shrink-0 rounded-full" class:active-dot={isActive('/')}></span>
			ダッシュボード
		</a>
		<a href="/past-exams" class="flex items-center gap-2.5 rounded-lg px-[10px] py-2 text-[13.5px] text-ink-2 no-underline" class:active={isActive('/past-exams')}>
			<span class="size-1.5 shrink-0 rounded-full" class:active-dot={isActive('/past-exams')}></span>
			過去問一覧
		</a>

		{#if pinnedSubjects.length > 0}
			<div class="mb-1.5 mt-4 px-[10px] text-[10.5px] uppercase tracking-[0.14em] text-ink-4">Pinned</div>
			{#each pinnedSubjects as subject}
				<a
					href="/subjects/{subject.id}"
					class="flex items-center gap-2.5 rounded-lg px-[10px] py-2 text-[13.5px] text-ink-2 no-underline"
	...
	<div class="mt-auto rounded-[10px] border border-hairline-soft bg-surface-1 px-3 py-[10px]">
